...
with right_col:
    ...
    if uploaded_file:
        ...
        risk_flag = 1 if prob >= threshold else 0
        ...
        st.markdown("<div class='card'>", unsafe_allow_html=True)
        st.markdown("<div class='metric-label'>Deployment Mode</div>", unsafe_allow_html=True)
        st.markdown(f"<div class='metric-value'>{mode}</div>", unsafe_allow_html=True)
        st.markdown("</div>", unsafe_allow_html=True)
        ...
        if risk_flag == 1:
            if mode == "Screening":
                ...
                st.warning("Elevated risk. Consider biopsy or further diagnostic imaging.")
        else:
            st.success("Low-risk finding. Consider follow-up imaging according to clinical guideline.")
