...
with tab1:
    ...
    st.markdown("### Case Assignment & SLA")
    ...
    if st.session_state.cases:
        ...
        assignee = st.text_input("Assign to Pathologist")

        if st.button("Assign Case"):
            st.session_state.assignments[selected_case] = assignee
            log_action(selected_case, f"Assigned to {assignee}")
            st.success("Assigned")
...
with tab2:
    ...
    st.markdown("### AI Performance Drift")

    if st.session_state.cases:

        drift_data = [{
            "confidence": c.get("ai_confidence", 0),
            "time": datetime.now()
